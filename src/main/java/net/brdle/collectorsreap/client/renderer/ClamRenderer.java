...
public class ClamRenderer extends MobRenderer<Clam, ClamModel> {
	...
	public @NotNull ResourceLocation getTextureLocation(@NotNull Clam entity) {
		...
		if (TEXTURES[variant] == null) {
			...
			if (!Minecraft.getInstance().getResourceManager().getResource(loc).isPresent()) {
				loc = ResourceLocation.fromNamespaceAndPath(CollectorsReap.MODID, "textures/entity/clam/clam_0.png");
			}
	...
	@Override
	protected void setupRotations(@NotNull Clam entityLiving, @NotNull PoseStack stack, float pAgeInTicks, float pRotationYaw, float pPartialTicks, float pBodyRot) {
		super.setupRotations(entityLiving, stack, pAgeInTicks, pRotationYaw, pPartialTicks, pBodyRot);
	}
}